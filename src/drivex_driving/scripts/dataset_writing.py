...
def save_dataset(date, info_data, data_path, driving_log):
    ...
    driving_log.to_csv(
        data_path + "/driving_log.csv", mode="a", index=False, header=False
    )
    info_data["dataset"]["image_number"] = len(list(os.listdir(data_path + "/IMG/")))

    info_data["dataset"]["date"] = (
        date + " until " + datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    )
...
def main():
    ...
    driving_path = rospack.get_path("drivex_driving")
    data_path = (
        f"{driving_path}/datasets/"
        + env
        + "-"
        + datetime.now().strftime("%d-%m-%Hh%Mm%Ss")
    )
    ...
    while not rospy.is_shutdown():
        ...
        image_name = (
            f"{str(curr_time.year)}_{str(curr_time.month)}_{str(curr_time.day)}_"
        ...
        row = pd.DataFrame(
            data=[[image_name, config["angular"], config["linear"]]],
        ...
        img_rgb_resize = cv2.resize(
            config["img_rgb"], dim, interpolation=cv2.INTER_AREA
        )
