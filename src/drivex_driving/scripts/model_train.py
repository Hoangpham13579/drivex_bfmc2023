...
def main():
    ...
    parser.add_argument(
        "-v", "--visualize", action="store_true", help="Visualize the loss"
    )
    parser.add_argument(
        "-d",
        "--dataset_name",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="folder name where the model is stored & it's also the model's name",
    )
    parser.add_argument(
        "-sm",
        "--summarize_model",
    ...
    parser.add_argument(
        "-batch_size", "--batch_size", default=16, type=int, help="Batch size"
    )
    parser.add_argument(
        "-c", "--cuda", default=0, type=int, help="Number of cuda device"
    )
    parser.add_argument(
        "-loss_t",
        "--loss_threshold",
    ...
    parser.add_argument(
        "-wd", "--weight_decay", type=float, default=0, help="L2 regularizer"
    )
    parser.add_argument(
        "-nw",
        "--num_workers",
    ...
    # General Path
    files_path = os.environ.get("DRIVEX_DRIVING")
    # Image dataset paths
    ...
    print(
        f"You are training using the device: "
        + Fore.YELLOW
        + f"{device}"
        + Style.RESET_ALL
    )
    ...
    if args["visualize"]:  # Checks if the user wants to visualize the loss
        ...
        loss_visualizer.draw(
            [0, maximum_num_epochs],
            [termination_loss_threshold, termination_loss_threshold],
            layer="threshold",
            marker="--",
            markersize=1,
            color=[0.5, 0.5, 0.5],
    ...
    if os.path.exists(folder_path):  # Checks to see if the model exists
        print(
            Fore.YELLOW
            + f"Model already exists! Do you want to resume training?"
            + Style.RESET_ALL
        )
        ans = input(
            Fore.YELLOW
            ...
            + Style.RESET_ALL
            + "verwrite: "
        )  # Asks the user if they want to resume training
    ...
    while True:
        ...
        for batch_idx, (image_t, label_t) in tqdm(
            enumerate(loader_train),
            total=len(loader_train),
            desc=Fore.GREEN
            + "Training batches for Epoch "
            + str(idx_epoch)
            + Style.RESET_ALL,
        ):
        ...
        if args["visualize"]:
            loss_visualizer.draw(
                list(range(0, len(epoch_train_losses))),
                epoch_train_losses,
                layer="train loss",
                marker="-",
                markersize=1,
            ...
            loss_visualizer.draw(
                list(range(0, len(epoch_test_losses))),
                epoch_test_losses,
                layer="test loss",
                marker="-",
                markersize=1,
        ...
        print(
            f"{Fore.LIGHTBLUE_EX}Epoch {str(idx_epoch)} Train Loss: {str(epoch_train_loss)} Test Loss: {str(epoch_test_loss)} With learning rate: {str(scheduler.get_last_lr())} {Style.RESET_ALL}"
        )
        ...
        if idx_epoch >= maximum_num_epochs:
            print(
                Fore.CYAN
                + "Finished training. Reached maximum number of epochs. Comparing to previously stored model"
                + Style.RESET_ALL
            )
            if epoch_test_loss < stored_test_loss:
                ...
                SaveGraph(
                    epoch_train_losses, epoch_test_losses, folder_path, last_saved_epoch
                )  # Saves the graph
                SaveModel(
                    model,
                    idx_epoch,
                ...
                print(
                    Fore.BLUE
                    + "Not saved, current loos "
                    + str(epoch_test_loss)
                    + ". Previous model is better, previous loss "
                    + str(stored_test_loss)
                    + "."
                    + Style.RESET_ALL
                )
            break
            ...
            print(
                Fore.CYAN
                + "Finished training. Reached target loss. Comparing to previously stored model"
                + Style.RESET_ALL
            )
            if epoch_test_loss < stored_test_loss:
                last_saved_epoch = idx_epoch
        ...
        if idx_epoch % 10 == 0:
            ...
            if (
                epoch_test_loss < stored_test_loss
            ):  # checks if the previous model is better than the new one
                last_saved_epoch = idx_epoch
                print(
                    Fore.BLUE
                    + "Saving model at Epoch "
